...
def validate_invoice(fiscal_id: str, inv: dict):
    ...
    if ins in (2, 3, 4):
        ...
        if not irtaxid:
            ...
            # شرط دوم ص۱۶ : روی یک مرجع نباید اصلاحی/برگشتی دیگری غیر از
            # ابطال‌شده صادر شده باشد. این یکی مستقل از نوع مرجع است — ارسال
            # ۲۲:۴۶ سندباکس روی یک مرجعِ *اصلی* دقیقا به همین دلیل رد شد.
