...
internal static class SpecFindings
{
    public static void Run(Action<string, bool, string> check, Action<string> line, bool withDb)
    {
        ...
        // نام این تست عمدا «آنچه ما می‌فرستیم» است نه «قاعده سامانه». در نصب
        // «مروارید» سامانه همین فیلدها را با مقدار صفر هم پذیرفت (۱٬۰۱۶ از
        // ۱٬۰۹۰ فاکتور با cap=insp=0)، پس این یک الزام نیست — فقط شکلی است که
        // این نصب می‌فرستد و سامانه رد نکرده.
        C("آنچه این نصب برای نقدی و نسیه می‌فرستد: cap + insp = مجموع صورتحساب",
