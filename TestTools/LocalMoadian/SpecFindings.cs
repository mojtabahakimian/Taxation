...
internal static class SpecFindings
{
    public static void Run(Action<string, bool, string> check, Action<string> line, bool withDb)
    {
        ...
        // این *ادعا* نیست، فقط ثبت است — و عمدا از C به line تبدیل شد.
        //
        // برای setm ۱ و ۲ سند هیچ قاعده‌ای ندارد (ص۴۵ جدول ۲۴ ردیف ۳ این دو فیلد
        // را فقط برای نقدی/نسیه اجباری می‌کند). پس هر نصبی می‌تواند شکل خودش را
        // داشته باشد و سامانه هر دو را می‌پذیرد:
        //   • یزدسپار : cap + insp = tbill   (۱٬۰۰۶ از ۱٬۰۳۳ فاکتور مالیات‌دار)
        //   • مروارید : cap = insp = 0       (۱٬۰۱۶ از ۱٬۰۹۰ فاکتور) — هم پذیرفته شد
        //
        // قبلا اینجا یک assert با آستانهٔ ۹۰٪ بود. آن assert روی دیتابیس یک نصب
        // دیگر قرمز می‌شد و ادعا می‌کرد چیزی خراب است، در حالی که فقط شکل آن نصب
        // فرق داشت. یک تست نباید تفاوت نصب‌ها را «شکست» گزارش کند.
        line($"      · شکل تسویهٔ این نصب: {vatOk} از {vatAll} فاکتور مالیات‌دارِ پذیرفته‌شده " +
             "رابطهٔ cap + insp = مجموع صورتحساب را دارند" +
             (vatAll > 0 && vatOk * 100 < vatAll * 50
                 ? " — یعنی این نصب مالیات را بیرون مبلغ تسویه می‌گذارد، که آن هم پذیرفته شده است"
                 : ""));
