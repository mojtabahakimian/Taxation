        ...
        private List<DRV_TBL> FetchInvoiceLines(long number, int tag)
        {
            ...
            else //دنافراز
            {
                sql = ($@"SELECT        dbo.HEAD_LST.NUMBER, dbo.HEAD_LST.TAG, dbo.HEAD_LST.DATE_N, dbo.INVO_LST.MABL, dbo.INVO_LST.MABL_K, dbo.INVO_LST.N_MOIN, dbo.INVO_LST.MABL_K - dbo.INVO_LST.N_MOIN AS mabkbt,
                                    dbo.INVO_LST.IMBAA, dbo.INVO_LST.MABL_K - dbo.INVO_LST.N_MOIN + dbo.INVO_LST.IMBAA AS mabkn, dbo.CUST_HESAB.MCODEM, dbo.CUST_HESAB.tob, dbo.INVO_LST.CODE, dbo.STUF_DEF.sstid, COALESCE((SELECT TOP 1 NAME_MO FROM dbo.TCOD_VAHED_EXTENDED WHERE IDD = NULLIF(dbo.STUF_DEF.mu, 0)),(SELECT TOP 1 NAME_MO FROM dbo.TCOD_VAHED_EXTENDED WHERE NAME_MO = dbo.TCOD_VAHEDS.NAMES),(SELECT TOP 1 NAME_MO FROM dbo.TCOD_VAHED_EXTENDED WHERE NAME_MO LIKE N'%' + dbo.TCOD_VAHEDS.NAMES + N'%'),dbo.TCOD_VAHEDS.NAMES) AS mu,
