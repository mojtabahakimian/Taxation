...
namespace Prg_Moadian.FUNCTIONS
{
    public static class CL_MOADIAN
    {
        ...
        public static void DoSendInvoice(string[] args)
        {
            ...
            {
                L_DRV_TBL_US.AddRange(dbms.DoGetDataSQL<DRV_TBL>($@"SELECT DATE_N, NUMBER1, HESAB, ADDRESS, TEL, CODE, ECODE, MCODEM, PCODE, IYALAT, CITY, KALA, MEGH, MABL, MABL_K, VNAMES, N_MOIN, mabkbt, IMBAA, mabkn, BARCODE, MEGHk, MOLAH, DEPART, DEPNAME, NUMBER,tob,DRVD_TBL.sstid,DRVD_TBL.mu,DRVD_TBL.vra, MEGH_MAR, N_KOL, JAY
            ...
            switch (_HEAD_EXTENDED.ins) //نوع صورت حساب
            {
                case 4: //از نوع برگشتی
                    ...
                    src_taxid = taxService.RequestTaxIdWithSpecificSerial(MemoryID, DtNowBase, NUMBER);
            ...
            foreach (var item in L_DRV_TBL_US)
            {
                ...
                if (item.N_KOL == 100 || item.JAY > 0)
                {
                    item.MABL_K = Math.Truncate((decimal)(item.MABL * item.MEGHk));
